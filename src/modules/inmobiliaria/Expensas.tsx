...
interface Expensa {
  id: string; propiedad_id: string; periodo: string; monto: number;
  monto_extraordinaria: number; estado: string; fecha_vencimiento: string | null;
  fecha_pago: string | null; observaciones: string | null;
...
const ESTADO_COLOR: Record<string, string> = { pendiente: '#F59E0B', pagada: '#10B981', vencida: '#EF4444' };

const emptyExpensa = {
  propiedad_id: '', periodo: new Date().toISOString().slice(0, 7), monto: 0,
  monto_extraordinaria: 0, estado: 'pendiente', fecha_vencimiento: '' as string | null,
  fecha_pago: null as string | null, observaciones: null as string | null,
...
export default function Expensas() {
  ...
  const [items, setItems] = useState<Expensa[]>([]);
  const [propiedades, setPropiedades] = useState<Propiedad[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [filterEstado, setFilterEstado] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [editing, setEditing] = useState<Expensa | null>(null);
  const [form, setForm] = useState(emptyExpensa);
  ...
  const loadData = async () => {
    ...
    const [eRes, pRes] = await Promise.all([
      supabase.from('inmobiliaria_expensas').select('*').eq('tenant_id', tenant!.id).order('periodo', { ascending: false }),
  ...
  const handleUpload = async (file: File) => {
    setUploading(true);
    const path = `expensas/${tenant!.id}/${Date.now()}_${file.name}`;
  ...
  const openNew = () => { setEditing(null); setForm({ ...emptyExpensa, periodo: new Date().toISOString().slice(0, 7) }); setWizardStep(0); setShowModal(true); };
  const openEdit = (e: Expensa) => { setEditing(e); setForm(e as any); setWizardStep(0); setShowModal(true); };
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      monto_extraordinaria: Number(form.monto_extraordinaria) || 0,
      fecha_vencimiento: form.fecha_vencimiento || null,
      fecha_pago: form.fecha_pago || null,
    };
    if (editing) {
      const { error } = await supabase.from('inmobiliaria_expensas').update(payload).eq('id', editing.id);
      if (!error) setItems(prev => prev.map(e => e.id === editing.id ? { ...e, ...payload } as Expensa : e));
    } else {
      const { data, error } = await supabase.from('inmobiliaria_expensas').insert({ ...payload, tenant_id: tenant!.id }).select().single();
      if (!error && data) setItems(prev => [data, ...prev]);
    }
    setShowModal(false);
  };

  const remove = (exp: Expensa) => {
    requestDelete('Esta acción eliminará la expensa y no se puede deshacer.', async () => {
      const { error } = await supabase.from('inmobiliaria_expensas').delete().eq('id', exp.id);
      if (!error) setItems(prev => prev.filter(e => e.id !== exp.id));
      setShowModal(false);
    });
  };
  ...
  const filtered = items.filter(e => {
    ...
    if (search && !propDir(e.propiedad_id).toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });

  const fmtMoney = (n: number) => `$${n.toLocaleString('es-AR')}`;
  const totalPendiente = items.filter(e => e.estado === 'pendiente').reduce((s, e) => s + e.monto + e.monto_extraordinaria, 0);
  const totalPagadas = items.filter(e => e.estado === 'pagada').reduce((s, e) => s + e.monto + e.monto_extraordinaria, 0);
  const totalVencidas = items.filter(e => e.estado === 'vencida').reduce((s, e) => s + e.monto + e.monto_extraordinaria, 0);

  if (loading) return <div style={{ padding: '2rem', color: 'var(--color-text-muted)' }}>Cargando expensas...</div>;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      ...
      <div className="module-header-desktop">
        <h1 style={{ fontSize: '1.25rem', fontWeight: 700 }}>Expensas</h1>
      ...
      <div style={{ display: 'flex', gap: isMobile ? 6 : 10 }}>
        {[
          ...
          { label: 'Pagadas', value: fmtMoney(totalPagadas), color: '#10B981', filter: 'pagada' },
          { label: 'Vencidas', value: fmtMoney(totalVencidas), color: '#EF4444', filter: 'vencida' },
        ].map(kpi => (
      ...
      {!isMobile && (
        <div style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border-subtle)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
          ...
          {filtered.map(e => {
            ...
            const total = e.monto + (e.monto_extraordinaria || 0);
            return (
              <div key={e.id}
                ...
                {/* Concepto */}
                <div style={{ cursor: 'pointer', minWidth: 0 }} onClick={() => openEdit(e)}>
                  <div style={{ fontSize: '0.8125rem', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>Expensa {e.periodo}</div>
                  <div style={{ fontSize: '0.6875rem', color: 'var(--color-text-muted)' }}>
                    {e.monto_extraordinaria > 0 ? `Ord. ${fmtMoney(e.monto)} + Ext. ${fmtMoney(e.monto_extraordinaria)}` : 'Ordinaria'}
                  </div>
                </div>
                {/* Monto */}
                <div style={{ fontSize: '0.8125rem', fontFamily: 'var(--font-mono)', fontWeight: 700 }}>{fmtMoney(total)}</div>
                {/* Propiedad */}
                <div style={{ fontSize: '0.8125rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{propDir(e.propiedad_id)}</div>
                {/* Estado */}
                <div>
                ...
                <div style={{ fontSize: '0.6875rem', color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center' }}>
                  {e.fecha_vencimiento ? new Date(e.fecha_vencimiento + 'T12:00:00').toLocaleDateString('es-AR', { day: 'numeric', month: 'short' }) : '—'}
                  {e.comprobante_url && <a href={e.comprobante_url} target="_blank" rel="noopener noreferrer" onClick={ev => ev.stopPropagation()} style={{ marginLeft: 4 }}><FileText size={12} color="#3B82F6" /></a>}
                </div>
                {/* Acciones */}
                <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                  <button onClick={() => openEdit(e)} style={{ ...iconBtn, color: 'var(--color-text-muted)' }}
                    onMouseEnter={ev => (ev.currentTarget.style.background = 'var(--color-bg-hover)')}
                    onMouseLeave={ev => (ev.currentTarget.style.background = 'var(--color-bg-surface)')}>
                    <Eye size={14} />
                  </button>
                  <button onClick={() => remove(e)} style={{ ...iconBtn, color: '#EF4444' }}
          ...
          {filtered.length === 0 && <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--color-text-muted)' }}>Sin expensas</div>}
        </div>
      )}
      ...
      {isMobile && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {filtered.map(e => {
            const color = ESTADO_COLOR[e.estado] || '#6B7280';
            return (
              <div key={e.id} onClick={() => openEdit(e)} style={{ padding: '12px 14px', borderRadius: 12, background: 'var(--color-bg-card)', border: '1px solid var(--color-border-subtle)', cursor: 'pointer' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                  <div style={{ fontWeight: 600, fontSize: '0.875rem', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{propDir(e.propiedad_id)}</div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)' }}>{e.periodo}</span>
                  <span style={{ fontSize: '0.875rem', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>{fmtMoney(e.monto + e.monto_extraordinaria)}</span>
                </div>
              </div>
            );
          })}
      ...
      {/* ─── WIZARD MODAL ─── */}
      {showModal && (() => {
        const STEPS = [{ label: 'Propiedad' }, { label: 'Montos' }, { label: 'Estado' }];
        const totalSteps = STEPS.length;
        const canNext = wizardStep === 0 ? !!form.propiedad_id : wizardStep === 1 ? form.monto > 0 : true;
        ...
        return (
          ...
          <div className="wizard-card" onClick={ev => ev.stopPropagation()}>
            <div className="wizard-header">
              <h3>{editing ? 'Editar expensa' : 'Nueva expensa'}</h3>
            ...
            <div className="wizard-body">
              ...
              {wizardStep === 1 && (<>
                <div className="wizard-row">
                  <div className="wizard-field">
                    <label className="form-label">Expensa ordinaria *</label>
                    <input type="number" className="form-input" value={form.monto || ''} onChange={e => setForm(f => ({ ...f, monto: Number(e.target.value) }))} placeholder="0" />
                  </div>
                  <div className="wizard-field">
                    <label className="form-label">Extraordinaria</label>
                    <input type="number" className="form-input" value={form.monto_extraordinaria || ''} onChange={e => setForm(f => ({ ...f, monto_extraordinaria: Number(e.target.value) }))} placeholder="0" />
                  </div>
                </div>
                <div className="wizard-field">
              ...
              {wizardStep === 2 && (<>
                ...
                {form.comprobante_url && form.estado === 'pendiente' && (
                  <button type="button" onClick={() => setForm(f => ({ ...f, estado: 'pagada', fecha_pago: new Date().toISOString().slice(0, 10) }))}
